...
uint256 hashGenesisBlock("0xcf969da250ea22795c6e22487cc7a0a5e49cef492254dac3ed714ccbbf898a0b");
...
{
    // EB3Coin: IsDust() detection disabled, allows any valid dust to be relayed.
...
{
    // EB3Coin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "EB3Coin genesis block";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
	    ...
	    assert(block.hashMerkleRoot == uint256("0x87b2169d44e3b9f28158a0a12ab2f3ba383bf74f74b97b55e463e4568c3bec52"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xa6, 0xca, 0xbb, 0x9b };
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("EB3CoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("EB3CoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static EB3CoinMiner(CWallet *pwallet)
{
    printf("EB3CoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("EB3Coin-miner");
    ...
    try { loop {
        ...
        printf("Running EB3CoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("EB3CoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&EB3CoinMiner, pwallet));
}
